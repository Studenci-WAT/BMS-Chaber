...
import pl.edu.wat.mspw_frontend.readcontrollers.TableTypKlimatuController;
...
public class InputTypKlimatuController {
    private TypKlimatuService typKlimatuService = new TypKlimatuService();
    private TableTypKlimatuController tableController;
    ...
    public void initialize() {
        InputControllerStatic.setupTitle(labelTitle, "Wprowadzanie nowego typu klimatu");
        setupDynamicControls();
        loadTableView(TableViews.TABLE_TYP_KLIMATU.getValue());
    ...
    private void addButtonAction() {
        ...
        try {
            if (!InputControllerStatic.isAnyControlEmpty(dynamicControls)) {
                typKlimatuService.create(
                        TypKlimatuDto.builder()
                                .nazwa(nazwa)
                                .skrot(skrot)
                                .build()
                );
    ...
    private void deleteButtonAction() {
        TypKlimatuDto selectedFireSys = tableController.getTableView().getSelectionModel().getSelectedItem();
        Stage stage = (Stage) deleteButton.getScene().getWindow();
        if (selectedFireSys != null) {
            try {
                typKlimatuService.delete(Long.valueOf(selectedFireSys.getId()));
