...
import pl.edu.wat.mspw_frontend.readcontrollers.TableTypTerenuWidoczController;
...
public class InputTypTerenuWidoczController {
    private TypTerenuWidoczService typTerenuWidoczService = new TypTerenuWidoczService();
    private TableTypTerenuWidoczController tableController;
    ...
    public void initialize() {
        InputControllerStatic.setupTitle(labelTitle, "Wprowadzanie nowego typu terenu widoczności");
        setupDynamicControls();
        loadTableView(TableViews.TABLE_TYP_TERENU_WIDOCZ.getValue());
    ...
    private void addButtonAction() {
        ...
        try {
            if (!InputControllerStatic.isAnyControlEmpty(dynamicControls)) {
                typTerenuWidoczService.create(
                        TypTerenuWidoczDto.builder()
                                .nazwa(nazwa)
                                .skrot(skrot)
                                .build()
                );
    ...
    private void deleteButtonAction() {
        TypTerenuWidoczDto selectedFireSys = tableController.getTableView().getSelectionModel().getSelectedItem();
        Stage stage = (Stage) deleteButton.getScene().getWindow();
        if (selectedFireSys != null) {
            try {
                typTerenuWidoczService.delete(Long.valueOf(selectedFireSys.getId()));
