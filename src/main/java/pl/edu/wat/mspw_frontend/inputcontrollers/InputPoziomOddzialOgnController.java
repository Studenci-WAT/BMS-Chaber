...
import pl.edu.wat.mspw_frontend.readcontrollers.TablePoziomOddzialOgnController;
...
public class InputPoziomOddzialOgnController {
    private PoziomOddzialOgnService poziomOddzialOgnService = new PoziomOddzialOgnService();
    private TablePoziomOddzialOgnController tableController;
    ...
    public void initialize() {
        InputControllerStatic.setupTitle(labelTitle, "Wprowadzanie nowego poziomu oddziaływania ogniowego");
        setupDynamicControls();
        loadTableView(TableViews.TABLE_POZIOM_ODDZIAL_OGN.getValue());
    ...
    private void addButtonAction() {
        ...
        try {
            if (!InputControllerStatic.isAnyControlEmpty(dynamicControls)) {
                poziomOddzialOgnService.create(
                        PoziomOddzialOgnDto.builder()
                                .nazwa(nazwa)
                                .skrot(skrot)
                                .build()
                );
    ...
    private void deleteButtonAction() {
        PoziomOddzialOgnDto selectedFireSys = tableController.getTableView().getSelectionModel().getSelectedItem();
        Stage stage = (Stage) deleteButton.getScene().getWindow();
        if (selectedFireSys != null) {
            try {
                poziomOddzialOgnService.delete(Long.valueOf(selectedFireSys.getId()));
