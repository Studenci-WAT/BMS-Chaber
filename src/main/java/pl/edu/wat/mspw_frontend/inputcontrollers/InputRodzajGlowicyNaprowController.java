...
import pl.edu.wat.mspw_frontend.readcontrollers.TableRodzajGlowicyBojController;
...
public class InputRodzajGlowicyNaprowController {
    private RodzajGlowicyNaprowService rodzajGlowicyNaprowService = new RodzajGlowicyNaprowService();
    private TableRodzajGlowicyNaprowController tableController;
    @FXML
    ...
    public void initialize() {
        InputControllerStatic.setupTitle(labelTitle, "Wprowadzanie nowego rodzaj głowicy naprowadzającej");
        setupDynamicTextFields();
        loadTableView(TableViews.TABLE_RODZAJ_GLOWICY_NAPROW.getValue());
    ...
    private void addButtonAction(){
        ...
        try {
            if (!InputControllerStatic.isAnyControlEmpty(dynamicControls)) {
                rodzajGlowicyNaprowService.create(
                        RodzajGlowicyNaprowDto.builder()
                                .nazwa(nazwa)
                                .skrot(skrot)
                                .build()
                );
    ...
    private void deleteButtonAction() {
        RodzajGlowicyNaprowDto selectedType = tableController.getTableView().getSelectionModel().getSelectedItem();
        Stage stage = (Stage) deleteButton.getScene().getWindow();
        if (selectedType != null) {
            try{
                rodzajGlowicyNaprowService.delete(Long.valueOf(selectedType.getId()));
