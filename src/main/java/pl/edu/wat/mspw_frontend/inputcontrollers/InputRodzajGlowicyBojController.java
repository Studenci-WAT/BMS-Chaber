...
public class InputRodzajGlowicyBojController {
    private RodzajGlowicyBojService rodzajGlowicyBojService = new RodzajGlowicyBojService();
    private TableRodzajGlowicyBojController tableController;
    @FXML
    ...
    public void initialize() {
        InputControllerStatic.setupTitle(labelTitle, "Wprowadzanie nowego rodzaj głowicy bojowej");
        setupDynamicTextFields();
        loadTableView(TableViews.TABLE_RODZAJ_GLOWICY_BOJ.getValue());
    ...
    private void addButtonAction(){
        ...
        try {
            if (!InputControllerStatic.isAnyControlEmpty(dynamicControls)) {
                rodzajGlowicyBojService.create(
                        RodzajGlowicyBojDto.builder()
                                .nazwa(nazwa)
                                .skrot(skrot)
                                .build()
                );
    ...
    private void deleteButtonAction() {
        RodzajGlowicyBojDto selectedType = tableController.getTableView().getSelectionModel().getSelectedItem();
        Stage stage = (Stage) deleteButton.getScene().getWindow();
        if (selectedType != null) {
            try{
                rodzajGlowicyBojService.delete(Long.valueOf(selectedType.getId()));
