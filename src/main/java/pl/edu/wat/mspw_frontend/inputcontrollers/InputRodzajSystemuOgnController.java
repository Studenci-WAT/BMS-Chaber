...
import pl.edu.wat.mspw_frontend.readcontrollers.TableRodzajSystemuOgnController;
...
public class InputRodzajSystemuOgnController {
    private RodzajSystemuOgnService rodzajSystemuOgnService = new RodzajSystemuOgnService();
    private TableRodzajSystemuOgnController tableController;
    @FXML
    ...
    public void initialize() {
        InputControllerStatic.setupTitle(labelTitle, "Wprowadzanie nowego rodzaju systemu ogniowego");
        setupDynamicTextFields();
        loadTableView(TableViews.TABLE_RODZAJ_SYSTEMU_OGN.getValue());
    ...
    private void addButtonAction(){
        ...
        try {
            if (!InputControllerStatic.isAnyControlEmpty(dynamicControls)) {
                rodzajSystemuOgnService.create(
                        RodzajSystemuOgnDto.builder()
                                .nazwa(nazwa)
                                .skrot(skrot)
                                .build()
                );
    ...
    private void deleteButtonAction() {
        RodzajSystemuOgnDto selectedType = tableController.getTableView().getSelectionModel().getSelectedItem();
        Stage stage = (Stage) deleteButton.getScene().getWindow();
        if (selectedType != null) {
            try{
                rodzajSystemuOgnService.delete(Long.valueOf(selectedType.getId()));
